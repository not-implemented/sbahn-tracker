...
$vehicleInfos = $db->query('SELECT model, `number`, isModern, hasWiFi, IF(`lastSeenAt` < \'2021-02-22\' OR `lastSeenAt` IS NULL OR hasWiFi IS NULL OR isModern = 0 AND oldVehicleStatus != \'active\', 1, 0) AS isOutdated FROM vehicle_info')->fetchAll();
